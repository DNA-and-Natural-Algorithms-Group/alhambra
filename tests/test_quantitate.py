...
from alhambra.quantitate import (
    Q_,
    D,
    nM,
    uM,
...
def test_dilute():
    assert dilute(target_conc='1 uM', start_conc='10 uM', vol='100 uL') == Q_(D(900), 'uL')
    assert dilute(target_conc='1 uM', start_conc='100 uM', vol='100 uL') == Q_(D(9900), 'uL')
    assert dilute(target_conc='1 uM', start_conc='100 uM', vol='100 uL') == Q_(D("9.9"), 'mL')
...
def test_measure_conc_from_specs():
    ...
    relerror = D('0.1')  # relative error needs to be Decimal to avoid type error in assert_approx below
